...
class _ParfumPageState extends State<ParfumPage> {
  ...
  void _showForm({Perfume? parfum}) {
    final isEdit = parfum != null;
    final ctrl = TextEditingController(text: parfum?.name ?? '');
            ...
            children: [
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Contoh: Lavender, Rose, Vanilla...',
                  prefixIcon: Padding(
                    padding: const EdgeInsets.all(12),
                    child: Image.asset('assets/parfume.png', width: 20, height: 20),
                  ),
                  filled: true,
                  ...
                  Expanded(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () {
                        ...
                        final vm = context.read<PerfumeVM>();
                        if (isEdit) {
                          vm.update(parfum.id, nama);
                        } else {
                          vm.add(nama);
                        }
                      ...
                      label: Text(
                        isEdit ? 'Simpan Perubahan' : 'Tambah Parfum',
                        style: const TextStyle(
                            color: Colors.white,
                            fontWeight: FontWeight.bold),
                      ),
  ...
  // ── Konfirmasi Hapus ─────────────────────────────────────────────────────────
  void _confirmDelete(BuildContext ctx, Perfume parfum) {
    showDialog(
      context: ctx,
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ctx.read<PerfumeVM>().delete(parfum.id);
              Navigator.pop(ctx);
            },
            style: ElevatedButton.styleFrom(
